...
class Plugin_DotKernel_MailTransporter extends Plugin_Abstract
{
	...
	{
		foreach($this->_options['transporter'] as $transporter)
		{
			if($transporter['enable'] == true)
			{
				// Here is a sample of how we link with the actual plugin
				// This is only a Plugin Handler Class
				$transporterObject = Plugin_DotKernel_MailTransporter_Factory::createTransporter($this->_options['smtpActive'], 
										$this->_options['siteEmail'], $transporter);
